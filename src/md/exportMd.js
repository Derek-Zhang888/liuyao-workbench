...
const FM_GUIDE = [
  ...
  '#   应期对错: 对                 同上',
  '#   方位对错: 对                 同上',
  '#   取数: 三                    v1.3.0 数量占应/射覆取数；无取数写 ""',
  '#   取数反馈: 神准               v1.3.0 取数应验：神准 / 相近 / 错；未定写 ""',
...
export function guashiToMd(g) {
  ...
  const fm = [
    ...
    ['取数', yamlScalar(rec.quShu ?? '')], // v1.3.0：取数文本框（旧记录无此字段导出为空）
    ['取数反馈', yamlScalar(rec.quShuFb ?? '')], // v1.3.0：取数反馈三档（神准/相近/错）
