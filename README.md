...
**版本 1.4.0** · React 18 + Vite 5 + Tailwind CSS · IndexedDB 零后端
...
| ② 占断 | 断语 / 应期 / 方位 / 取数 / 反馈 / 吉凶六栏，与盘面同屏一气呵成 |
| ③ 反馈 | 已反馈对 / 错 + 取数三档（神准 / 相近 / 错）自动归入正确率统计；待反馈 / 待占断独立标记 |
| ④ 复盘 | 四维正确率（吉凶 / 应期 / 方位 / 取数）+ 正确率趋势 + 标签聚合 + 时间筛选，数字点击直接钻回卦例库 |

> 市面产品大多止步于①②。四维正确率 + 复盘闭环，是本项目区别于所有同类产品的根本。
...
### 📊 多维度统计与卦例库筛选
- **统计页**：四卡总览（总卦例 / 已反馈 / 待反馈 / 待占断）+ **四维正确率**（吉凶 / 应期 / 方位 / 取数）+ **取数卡**（三档 神准 / 相近 / 错 + 双口径 神准率 / 神准+相近率）+ 标签多选筛选（严格模式=全部命中）+ 创建时间范围筛选，数字一键钻取卦例库
- **反馈结果筛选**：跨维度与、同维度或；**严格反馈**开关（只看反馈维度恰好等于所选维度的卦例）；选中维度卡片金边框高亮联动
- **四色正确率趋势**：吉凶绿 / 应期蓝 / 方位金 / 取数紫，按创建时间分组的正确率折线；当月 / 累计采样切换、样本 <3 空心点防误导、维度显隐、点按节点查看正确率
- **卦例库**：状态三态筛选（待占断 / 待反馈 / 已反馈）+ 维度子筛选（吉 / 凶 / 应期 / 方位 / 取数）+ 对错多选筛选 + 创建时间范围 + 关键字搜索（标题 / 断语）+ 标签多选 + 排序；多选批量导出 md / 批量删除 / 回收站
...
- **587 项自动化测试全绿**，覆盖排盘引擎 / 干支历法 / 神煞 / 真太阳时 / 画板 / Markdown 双向 / 统计与页面交互
- Vite 生产构建 0 警告
